...
class ScoreCalc
{
    public function calculateScore(Hand $hand): int
    {
        $score = 0;
        $jokers = 0;
        $aces = 0;
    ...
    {
        // Add possible joker values to all scores, get rid of duplicates, and find best value.
        if ($jokers === 0) {
            return $scores;
        }
        $finalScores = [];
    ...
    {
        $playerScore = $finalScores[array_key_last($finalScores)];
        foreach ($finalScores as $scr) {
            if ($scr > TWENTY_ONE) {
                break;
            }
            $playerScore = $scr;
