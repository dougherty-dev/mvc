...
class __TwigTemplate_613ba043a5cd782a09b993562b744cb8 extends Template
{
    ...
    <section class=\"two-columns\" id=\"kmom10\">
        <h2>Kmom10</h2>
        <h3>1–3. Webbplats</h3>
        <p class=\"initcap\">För slutprojektet har valts en tillämpning av kortspelet Texas Hold’em. För ändamålet gjordes en klar avgränsning från tidigare kursmoment genom att lägga till en ny entrypoint «project» i webpacks konfigurationsfil, varvid en separat sajt med skilda stilmallar kan byggas. Klichén har att poker renderas i gröna färger, men temat på den kulören är redan upptaget i den ordinarie utformningen i Ghibli-stil. Istället valdes en stark kontrast i blå, lila, rosa och rött, ett tema som faktiskt passar utmärkt för ändamålet. Ytterligare relief ges av en antikva (Bembo) som typsnitt i motsättning till den tidigare linjären (Poppins).</p>
        <p>Huvud och fot har bantats ned betänkligt för att ge plats åt pokerbordet vad det lider, samtidigt som en unik navigeringslist har implementerats. Referens till huvudsajten ges av en hem-ikon (🏠) i hörnet. Själva innehållet består av en frontsida med regelbeskrivning för och en länk till spelet, samt en om-sida som beskriver projektet i korthet. Repo, PHPDoc med mera följer tidigare mönster, och erbjuder inget nytt.</p>
        <h3>4. JSON-API</h3>
        <p>I likhet med tidigare API:n för kortspelet 21 ges här några motsvarigheter för Texas Hold’em. Bland annat ges åtkomst till aktuell ställning i spelet, med öppna kort för varje spelare och samtliga övriga data i en ganska komplex struktur. En enklare variant är att välja (POST) en enskild spelares aktuella hand i klarvy, samt en motsvarighet som ger alla tre spelares händer och den gemensamma handen. Ytterligare en rutt leder till aktuell kortlek samt vilka kort som har kastats (bränts), för den som tror sig kunna dra några slutledningar av det. Det mest intressanta gränssnittet ger de tre spelarnas bästa möjliga händer under aktuell ställning på bordet, under förutsättning att det finns gemensamma kort på bordet så att en giltig hand kan formas – för sju kort (2 + 5) finns 21 möjliga kombinationer att välja från, eller inalles 63 kombinationer för tre spelare.</p>
        <h3>5. ORM</h3>
        <p>Tidigare kortspel nyttjade sessionen för kommunikation emllan back- och frontend, men här nyttjas istället en databas för en mer beständig form. Man kan alltså växla browser och enhet för att fortsätta med spelet, i vad som är ett embryo till en mer avancerad lösning med konton och flera spel. Doctrines system nyttjades för att skapa två enkla tabeller <code>community</code> och <code>player</code>, där den förra håller variabler kring spelet och aktuell runda, och den senare lagrar motsvarande data för varje enskild spelare. Några kopplingar ges inte i denna tillämpning. Även en tabell <code>log</code> har skapats, men den är för närvarande inte i bruk – det skulle öka komplexiteten och ta för mycket tid i anspråk att implementera LoggerInterface.</p>
        ...
        <p>Tautologiskt har vi i korta drag följande utmaningar med projektet, samtliga relaterade till kortspelets betydligt högre komplexitet. <strong>Dels</strong> att forma ett plastiskt pokerbord i CSS så att spelet – i vart fall i princip – kan spelas även på den minsta tänkbara enhet. Knappar för croupier och mörkar samt ikoner för spelare måste flyta med och skalas korrekt i takt med att bordet förändrar dimensioner, och på samma sätt måste händer med kort ligga fast tillsammans med insatser. Här nyttjades främst <code>grid</code> i <code>grid</code> med namngivna areor för att definiera upp bordet.</p>
        <p><strong>Dels</strong> att implementera den omfattande logik som präglar spelet, under samtidigt beaktande av att hålla klasserna små, rena och fria från komplexitet. Komposition (framför arv) med beroendeinjektion har nyttjats flitigt för att åstadkomma kod med ganska hög grad av återanvändning. Exempelvis är det bara petitesser som skiljer budgivningen åt under en runda (preflop, flop, turn, river), varför en och samma huvudslinga kan nyttjas för ändamålet.</p>
        <p><strong>Dels</strong> att jämka samman databas med modeller samt skyffla relevanta data fram och tillbaka. I retrospektiv hade ett bättre namngivningsschema varit bättre, så att man inte ständigt förväxlar <code>players</code> och <code>community</code> för respektive modell. Utrymmet för optimering är gott i allmänhet, och i synnerhet i fråga om att styra upp databashanteringen, men det ligger nog utanför de marginaler som definieras av projektet.</p>
        <p><strong>Dels</strong> att hålla ned ett växande antal klasser i storlek och komplexitet för att tillfredsställa Scrutinizer och PhpMetrics, ett förfarande som dessutom gör testningen mer omständlig. Den komplexa logiken är även i detta avseende en utmaning.</p>
        <h3>Allmänt</h3>
        <p>Projektet är det som har tagit överlägset mest tid i anspråk bland kurser, men det är förstås självförvållat genom att ta på sig en mer utmanande uppgift. Svårigheten och tidsåtgången har framförallt legat i att finna en fungerande lösning för logiken under budgivningen, en process som ideligen har bjudit på motgångar och tidskrävande felsökning. Det är ett rimligt projekt för den som har ambitioner, men jag tror att många inte skulle gå i land med det.</p>
        <h3>Kursen</h3>
        <p>Överlägset mest intressant och viktigast av kurser som har getts, och kanske den som mest liknar verkligheten för backendutvecklare. Även för den med någon bakgrund i PHP är det en högst relevant kurs, särskilt om man saknar tidigare erfarenhet av ramverk. Men samtidigt är kurserna vid Webtec oerhört mastiga, samtidigt som dokumentationskraven är något överdrivna – inga problem för den som har ordets gåva och har bakgrund i att författa digra mängder text, men nog ett hinder i största allmänhet för mången annan. Å andra sidan är omfattningen och drillen av vikt för att verkligen lära sig objektorienterad webbprogrammering. Betyg blir 11/10, eller «stabilt och felfritt».</p>
        <p></p>
        <p></p>
        <p></p>
        <figure>
            <img src=\"";
        // line 40
        yield $this->env->getRuntime('Twig\Runtime\EscaperRuntime')->escape($this->extensions['Symfony\Bridge\Twig\Extension\AssetExtension']->getAssetUrl("build/images/chihiro-kmom10.avif"), "html", null, true);
        yield "\" width=\"1536\" alt=\"Chihiro spelar Texas Hold’em\">
        </figure>
    </section>
...
";
    ...
    {
        return array (  244 => 124,  228 => 111,  210 => 96,  190 => 79,  174 => 66,  159 => 54,  150 => 48,  144 => 45,  136 => 40,  100 => 6,  87 => 5,  64 => 3,  41 => 1,);
    }
